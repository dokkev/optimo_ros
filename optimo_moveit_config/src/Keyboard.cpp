...
// This file is not actually created by us, but the autogenerated header replaces this part so here
...
// Some constants used in the Servo Tele-operation demo
const std::string TWIST_TOPIC = "/optimo/servo/ee_twist_desired";
...
private:
  ...
  std::chrono::system_clock::time_point last_pub;
  ...
  nh_(rclcpp::Node::make_shared("servo_keyboard_input"))
{
  ...
  timer = nh_->create_wall_timer(
    std::chrono::milliseconds(50), std::bind(&KeyboardServo::pub_empty, this));
}
...
void KeyboardServo::pub_empty()
{
  // if message have not been published for ~50 ms, publish a zero command.

  if (
    std::chrono::duration_cast<std::chrono::duration<double>>(
      std::chrono::system_clock::now() - last_pub)
      .count() > 0.05) {
    auto joint_msg = std::make_unique<control_msgs::msg::JointJog>();
    joint_msg->header.stamp = nh_->now();
    joint_msg->header.frame_id = BASE_FRAME_ID;
    joint_msg->joint_names.push_back("joint7");
    // there is a bug in servo where starting by publishing zeros results in the zero position
    // being commanded
    joint_msg->velocities.push_back(DBL_MIN);
...
int KeyboardServo::keyLoop()
{
  ...
  for (;;) {
    ...
    if (publish_twist) {
      ...
      publish_twist = false;
      last_pub = std::chrono::system_clock::now();
