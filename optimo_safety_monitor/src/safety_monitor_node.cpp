...
: Node("safety_monitor", options)
{
  ...
  RCLCPP_INFO(get_logger(),
    "Safety monitor started (rate=%.1f Hz, force_threshold=%.1f N)",
    check_rate_hz, force_threshold_);
}
...
{
  // DEMO: trigger unsafe after 5 seconds from node start.
  // TODO: Replace with real force safety logic, e.g.:
  //   std::lock_guard<std::mutex> lock(mutex_);
  //   if (!wrench_received_) return true;
  //   double fx = latest_wrench_.wrench.force.x;
  //   double fy = latest_wrench_.wrench.force.y;
  //   double fz = latest_wrench_.wrench.force.z;
  //   double magnitude = std::sqrt(fx*fx + fy*fy + fz*fz);
  //   return magnitude < force_threshold_;

  double elapsed = (now() - start_time_).seconds();
  if (elapsed > 5.0) {
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000,
      "Force unsafe (demo: %.1f s elapsed > 5.0 s)", elapsed);
    return false;
  }
  return true;
}
...
void SafetyMonitorNode::timer_callback()
{
  ...
  if (!safe && !stop_triggered_) {
    RCLCPP_WARN(get_logger(), "DANGER detected — calling stop_cb");
    trigger_stop();
  ...
  if (safe && stop_triggered_) {
    RCLCPP_INFO(get_logger(), "Safety restored");
