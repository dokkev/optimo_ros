...
#include "optimo_api/EEServoCallback.h"

namespace optimo_ros
{
std::mutex EEServoCallback::trajectory_mutex_;

EEServoCallback::EEServoCallback(
...
void EEServoCallback::calculate_torque(std::vector<int> & torque_)
{
  ...
  switch (task->get_status()) {
    case roboligent::CommandStatus::IDLE: {
      ...
      if (!sub_future.valid()) {
        sub_future = std::async(std::launch::async, &EEServoCallback::create_subscription, this);
      }
      if (sub_future.wait_for(std::chrono::seconds(0)) == std::future_status::ready) {
        if (!cartesian_sub || !pub) {
          RCLCPP_INFO(node->get_logger(), "EEServoCallback::calculate_torque: Failed to create subscription.");
          task->set_status(roboligent::CommandStatus::FAILURE);
          return;
        }
        RCLCPP_INFO(node->get_logger(), "EEServoCallback::calculate_torque: Subscription created.");

        trajectory = std::make_unique<roboligent::CartesianTrajectory>(model);
        trajectory->set_servo_filters(1, 5);
        servo_point = model.get_tcp_pose();
        last_target_pose = optimo_msgs::msg::PoseElbow();
        servo_point.push_back(model.get_elbow_pose().angle);
        RCLCPP_INFO(node->get_logger(), "EEServoCallback::calculate_torque: Trajectory created.");
...
void EEServoCallback::create_subscription()
{
  if (cartesian_sub) {
    cartesian_sub.reset();
  }
  ...
  cartesian_sub = node->create_subscription<optimo_msgs::msg::PoseElbow>(
    "/optimo/ee_pose_desired", 10, std::bind(&EEServoCallback::cartesian_cb, this, std::placeholders::_1));
...
void EEServoCallback::cartesian_cb(const optimo_msgs::msg::PoseElbow & msg)
{
  total_messages++;
  ...
  RCLCPP_INFO(node->get_logger(), "EEServoCallback::cartesian_cb: %d", is_different);
  ...
  if (is_different) {
    ...
    if (time_diff_ms > 200) {
      RCLCPP_WARN(node->get_logger(), "EEServoCallback::cartesian_cb: %ld ms since last unique message (message %d)", time_diff_ms, total_messages);
    }
...
void EEServoCallback::convert_pose_to_servo_point()
{
  if (
...
void EEServoCallback::joint_cb(const sensor_msgs::msg::JointState & msg)
{
